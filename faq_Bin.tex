...
apt-cache show: show the infomation of one package
...
ouput in the cfg file is not always the same. So, with the command, atomeye could store the atom data in the same
...
PS: However, it is possible to recover the files you deleted wich ''rm''. But it is not an easy thing.
...
-c: create\\
-r: append file\\
-v: ouput info when handle files\\
...
uncompress:    tar [-]xzvf this.tar.gz
...
\end{verbatim}
Tips: If you want caculate one formular with integer, it could be done like below:
\begin{verbatim}
echo $((2+3))
\end{verbatim}
...
\end{itemize}
PS: The vim-laxtex reference card makes a better summary. So I stop making notes.
...
\begin{verbatim}
find . -name '*' -print0 | xagrs -0 grep 'Loop time'  
\end{verbatim}
%----------------------------------------------------------------------------------------
...
\begin{verbatim}
pdfimages -j -p -f 2 -l 3 one.pdf nameprefix
\end{verbatim}
%----------------------------------------------------------------------------------------
...
\begin{verbatim}
rsync -avz -e 'ssh -p 2222' xubin@manager:PATH/ localpath
\end{verbatim}
...
Note that for LJ units, the default mode of thermodyamic output via the thermo\_style command is to normalize energies by
